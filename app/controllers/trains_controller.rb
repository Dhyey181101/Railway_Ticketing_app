class TrainsController < ApplicationController
  ...
  def filter_trains
    min_average_rating = params[:min_average_rating].to_f
    @trains = Train.where('average_rating >= ?', min_average_rating)
    render 'index' # You can render the index view with the filtered trains
  end
  ...
  private
    ...
    def train_params
      params.require(:train).permit(:train_number, :departure_station, :termination_station, :departure_date, :departure_time, :arrival_date, :arrival_time, :ticket_price, :max_seats, :available_seats)
    end
end
